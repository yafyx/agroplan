...
export default function Home() {
	...
	const handleSubmit = async () => {
		...
		const response = await fetch("https://agriplan-api.onrender.com/api/predict", {
			method: "POST",
			headers: {
	...
	return (
		<section className="flex flex-wrap items-center justify-center gap-4 p-8 sm:flex-nowrap sm:px-8 sm:gap-8 md:flex-nowrap md:gap-16 backdrop-blur-lg bg-white/50 dark:bg-zinc-900 rounded-2xl">
			<div className="flex flex-col flex-wrap w-full gap-4">
				<h1 className="text-xl font-bold sm:text-2xl md:text-4xl">Soil Nutrient Predict</h1>
				<p className="text-gray-500 dark:text-gray-400">Enter the soil parameters:</p>
				<Autocomplete
					variant="faded"
					...
					{cropOptions.map((option) => (
						<AutocompleteItem key={option.value} value={option.value.toString()}>
							{option.label}
						</AutocompleteItem>
					))}
				</Autocomplete>
			...
			{hasPrediction && !loading && (
				<div
					...
					<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
						Based on the soil parameters and crop selection, the predicted nutrient levels are:
					</p>
					<div className="p-2 text-xl font-medium bg-gray-200 rounded-lg sm:p-4 dark:bg-zinc-900/50">
						<div>
							N:
							<span className="text-4xl font-semibold">{prediction.predictions.N.toFixed(2)}</span>
						</div>
						<div>
							P:
							<span className="text-4xl font-semibold">{prediction.predictions.P.toFixed(2)}</span>
						</div>
						<div>
							K:
							<span className="text-4xl font-semibold">{prediction.predictions.K.toFixed(2)}</span>
						</div>
					</div>
					<div className="space-y-2">
						<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
							Based on the predicted nutrient levels and the selected crop (
							{cropOptions.find((option) => option.value.toString() === crop)?.label}
							), the following recommendations are provided:
						</p>
						<div className="p-2 bg-gray-200 rounded-lg sm:p-4 dark:bg-zinc-900/50">
							<table>
								<tbody>
